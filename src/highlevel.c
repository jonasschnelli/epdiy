...
EpdiyHighlevelState epd_hl_init(const EpdWaveform* waveform) {
    ...
    state.mirror_x = epd_get_display()->display_type == DISPLAY_TYPE_HORIZONTAL_MIRRORED;
...
/**
 * Mirror a line horizontally at the framebuffer level
 * Each byte contains 2 pixels (4 bits per pixel): [P1|P0]
 * For horizontal mirroring, we need to reverse the pixel order within the line
 */
__attribute__((optimize("O3"))) static inline void 
mirror_framebuffer_line_horizontal(uint8_t* line, int line_bytes) {
    // Lookup table for reversing 2 pixels within a byte
    // Maps [P1|P0] to [P0|P1] for 4-bit pixels
    static const uint8_t reverse_2px_lut[256] = {
    ...
    // Reverse bytes and reverse pixels within each byte
    while (start < end) {
...
void epd_hl_mirror_framebuffer_horizontal(uint8_t* framebuffer, int width, int height) {
    ...
    for (int y = 0; y < height; y++) {
        ...
        mirror_framebuffer_line_horizontal(line, line_bytes);
    }
}
...
enum EpdDrawError epd_hl_update_area(
    ...
    ESP_LOGI(
        "epdiy",
        "mirror: %dms, rot: %dms, diff: %dms, draw: %dms, buffer update: %dms, mirror_rev: %dms, total: %dms",
